...
import net.hero.genai.ollama.OllamaConfigController;
import net.hero.genai.security.SecurityService;
import net.hero.genai.security.SecuritySettingsController;
import net.hero.genai.supportai.SupportAiService;
...
import net.hero.genai.workflow.WorkflowStepStatus;
import net.hero.genai.workspace.MainWorkspaceController;
import net.hero.genai.workspace.WorkspaceFile;
...
public final class ChatController {
    ...
    private void handleSend() {
        ...
        if (isAskingForWorkflows) {
            ...
            final String response = SupportAiService.getInstance().invoke("list-workflows", null);
                ...
                if ("GATHERING".equalsIgnoreCase(result.status())) {
                    ...
                    if (result.fileAccessNeeded() && result.fileAccessPath() != null) {
                        ...
                        if (workspaceDir != null) {
                            ...
                            final String checkResult = SupportAiService.getInstance().invoke("security-check", checkArg);
                            final boolean permitted = "PERMITTED".equals(checkResult);

                            final String lookupResult = SupportAiService.getInstance().invoke("file-lookup", result.fileAccessPath());
        ...
        Platform.runLater(() -> {
            ...
            if (active != null) {
                ...
                for (int i = 0; i < active.steps().size(); i++) {
                    WorkflowStep step = active.steps().get(i);
                ...
                for (int i = 0; i < proposed.steps().size(); i++) {
                    WorkflowStep step = proposed.steps().get(i);
                    HBox hbox = new HBox(8);
                    hbox.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
    ...
    private void runActiveWorkflowStep() {
        ...
        WorkflowStep step = service.getActiveWorkflow().steps().get(service.getCurrentStepIndex());
